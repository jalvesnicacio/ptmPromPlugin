...
/**
 * This class creates a XEventClassifier. XEventClassifier can be used to impose
 * a classification of events. Two events which belong to the same event class
 * can be considered equal, i.e. to refer to the same higher-level concept they
 * represent (e.g., an activity). Event classes are imposed on a log by a
 * specific classifier. This class can be configured with such a classifier,
...
public class BeepBeepParameters
{
	...
	/**
	 * Returns whether these parameter values (classifier) are equal to the given
	 * parameter values.
	 * 
	 * @param object
